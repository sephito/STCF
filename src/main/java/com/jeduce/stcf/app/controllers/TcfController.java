...
@Controller
@Secured({ "ROLE_ADMIN" })
@RequestMapping("evaluar/oficiales")
...
public class TcfController {
	...
	public String periodoSelect(Model model) {
		model.addAttribute("titulo", "Seleccionar Periodo");
		List<Periodo> periodos = periodoService.findAll();
		model.addAttribute("periodo", new UsuPer());
		model.addAttribute("periodos", periodos);
		model.addAttribute("classActiveEvaluar", "active");
		return "tcf/periodo";
	}

	@RequestMapping(value = "/periodo", method = RequestMethod.POST)
	public String guardar(@Valid UsuPer periodo, Map<String, Object> model, RedirectAttributes flash,
			SessionStatus status, @ModelAttribute("periodo") UsuPer usuper) {

		if (periodo.getCodigo() != "" || periodo.getCodigo() != null) {

			model.put("titulo", "Listado de unidades pertenecientes al Periodo: " + usuper.getCodigo());
			List<Unidad> unidad = usuarioService.ByUnidad(periodo.getCodigo());
			model.put("unidades", unidad);
			model.put("classActiveEvaluar", "active");

			return "tcf/unidad";

		} else {
			flash.addFlashAttribute("error", "El Codigo de la unidad no puede ser vacio!");
	...
	public String verPersonal(@PathVariable(value = "id") String id, Map<String, Object> model,
			RedirectAttributes flash, @ModelAttribute("periodo") UsuPer usuper) {
		...
		if (id != "" || id != null) {
			...
			usuper.setUnidad(id);
			model.put("periodo", usuper);
			
			model.put("titulo", "Seleccionar el Tipo de Examen"+usuper.getCodigo()+""+usuper.getUnidad());
			
			return "tcf/tipoExamen";
		} else {
	...
	@RequestMapping(value = "unidad/tipoExamen/{id}")
	public String evaluarOficial(@PathVariable(value = "id") Long id, Map<String, Object> model,
			RedirectAttributes flash,@ModelAttribute("periodo") UsuPer usuper) {

		if (id > 0) {						
			
			
			usuper.setTipoExamen(id);
			model.put("periodo", usuper);
			model.put("titulo", "Seleccionar el Tipo de Examen"+usuper.getCodigo()+""+usuper.getUnidad()+" "+usuper.getTipoExamen());			
			List<TcfyDet> tcfydets = usuarioService.ByTcfyDetMM(usuper.getCodigo(),usuper.getUnidad(),usuper.getTipoExamen());
			...
			if (id==3) {	
				...
				model.put("titulo", "Listado de usuarios"+usuper.getCodigo()+usuper.getUnidad()+usuper.getUnidad()+usuper.getTipoExamen());		
				...
				return "tcf/menores_40";
			}
			if (id==4) return "tcf/mayores_40";
			if (id==1) return "tcf/menores_36";
			if (id==2) return "tcf/mayores_36";
			if (id==5) return "tcf/fuerzas_especiales";
			
			return "tcf/periodo";
			
			
		} else {
			flash.addFlashAttribute("error", "El Codigo del tipo de examen no puede ser vacio!");
			return "redirect:/periodo";
		}
	...
	public String listarMen40( Map<String, Object> model, @ModelAttribute("periodo") UsuPer usuper) {
